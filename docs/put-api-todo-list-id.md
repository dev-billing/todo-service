...
* 지정한 ID 의 Todo 항목을 전달된 값으로 전체 교체합니다 (구 경로).
* 가능하면 `/external/api/todo-list/{id}` (PATCH) 또는 `/internal/api/todo-list/update/{id}` 사용 권장.
...
```json
{
  ...
  "body": { "id": 42, "title": "수정", "status": "DONE" }
}
```
